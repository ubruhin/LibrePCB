...
SI_NetSegment::SI_NetSegment(Schematic& schematic, const DomElement& domElement) :
    SI_Base(schematic), mUuid(), mNetSignal(nullptr)
{
    try
    {
        ...
        // Load all netpoints
        foreach (const DomElement* node, domElement.getChilds("netpoint")) {
        ...
        // Load all netlines
        foreach (const DomElement* node, domElement.getChilds("netline")) {
        ...
        // Load all netlabels
        foreach (const DomElement* node, domElement.getChilds("netlabel")) {
...
void SI_NetSegment::getSelectedItems(QList<SI_Base*>& items,
                                     bool floatingPoints,
                                     bool attachedPoints,
...
void SI_NetSegment::serialize(DomElement& root) const
{
    if (!checkAttributesValidity()) throw LogicError(__FILE__, __LINE__);
